...
var template = require('jsdoc/template'),
    ...
    linkto = function(longname, linktext, cssClass) {
		return helper.linkto(longname, (linktext || longname).replace(/module:/, ''), cssClass);
	},
...
var getSignatureTypes = function(d, cssClass) {
    var types = [];
    ...
    return types;
}, getSignatureParams = function(d, optClass) {
    var pnames = [];
    ...
    return pnames;
}, getSignatureReturns = function(d, cssClass) {
    var returnTypes = [];
...
function addSignatureParams(f) {
    var params = getSignatureParams(f, 'optional');
...
function attachModuleSymbols(doclets, modules) {
    ...
    return modules.map(function(module) {
        if (symbols[module.longname]) {
            ...
            module.module.name = getClassName(module.module.name); //module.module.name.replace('module:', '') + '';
    ...
    if (members.modules.length) {
        ...
        members.modules.forEach(function(m) {
            if ( !hasOwnProp.call(seen, m.longname) ) {
                var parentModule = m.name.substring(0, m.name.lastIndexOf('/'));
                if(!hasOwnProp.call(modules, parentModule)) {
					modules[parentModule] = true;
                    if(!firstModule) {
                        nav += '</ul>';
                    }
                    nav += '<h4>'+parentModule+'</h4><ul class="nav nav-list">';
                    if(firstModule) {
                        firstModule = false;
                    }
				}
//                nav += '<li>'+linkto(m.longname, m.longname.substring(m.longname.lastIndexOf('/')+1))+'</li>';
                nav += '<li>'+linkto(m.longname, getClassName(m.longname))+'</li>';
            }
            seen[m.longname] = true;
        });

        if(!firstModule) nav += '</ul>';
    }

    if (members.externals.length) {
        nav += '<h3>Externals</h3><ul class="nav nav-list">';
    ...
    if (members.classes.length) {
        ...
        if (classNav !== '') {
            nav += '<h3>Classes</h3><ul class="nav nav-list">';
            nav += classNav;
            nav += '</ul>';
        }
    }
    ...
    if (members.events.length) {
        nav += '<h3>Events</h3><ul class="nav nav-list">';
    ...
    if (members.namespaces.length) {
        nav += '<h3>Namespaces</h3><ul class="nav nav-list">';
    ...
    if (members.mixins.length) {
        nav += '<h3>Mixins</h3><ul class="nav nav-list">';
    ...
    if (members.tutorials.length) {
        nav += '<h3>Tutorials</h3><ul class="nav nav-list">';
    ...
    if (members.globals.length) {
        ...
        if (!globalNav) {
            ...
            nav += '<h3>Global</h3><ul class="nav nav-list">' + globalNav + '</ul>';
        }
    }
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    data().each(function(doclet) {
        ...
        if (doclet.kind === 'class') {
            doclet.ancestors = getAncestorLinksForModules(doclet);
        }
    });

    function getAncestorLinksForModules(doclet, cssClass) {
        var ancestors = [],
            doc = doclet.augments;

        while (doc) {
            doc = find({longname: doc, kind: 'class'});
            if (doc) { doc = doc[0]; }
            if (!doc) { break; }
        ...
        if (ancestors.length) {
            ancestors[ancestors.length - 1] += (scopeToPunc[doclet.scope] || '');
        }
        return ancestors;
    }
    ...
    // generate urlmapping.json
    fs.writeFileSync(path.join(outdir, 'urlmapping.json'), JSON.stringify(helper.longnameToUrl), 'utf8');
